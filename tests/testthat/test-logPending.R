test_that("logPending accepts files in the QC log and returns expected errors", {
  ...
  with_demoRepo({
    ...
    if (!gitLog(file1)[["last_commit"]] %in% log[log$file == file1, "commit"]) {
      expect_true(file1 %in% pending$file)
    }
    
    if (!gitLog(file2)[["last_commit"]] %in% log[log$file == file2, "commit"]) {
      expect_true(file2 %in% pending$file)
    }
    
    if (gitLog(file3)[["last_commit"]] %in% log[log$file == file3, "commit"]) {
      expect_true(!file3 %in% pending$file)
    }
    
    if (!gitLog(file4)[["last_commit"]] %in% log[log$file == file4, "commit"]) {
