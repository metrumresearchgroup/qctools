...
outputs (ie. figures and tables).

## Table of Contents

-   Use cases: Leveraging version control vs local file comparisons
-   **`compareFigures()`**: Generates a html document showing two versions of a list of figures (pdf or png).
-   **`compareTables()`**: Generates a html document showing two versions of a list of tables (tex).
...
-   Comparisons of two local files (with different names)

Both functions have a `.path_previous` argument. If left empty, an html page will be generated comparing the
current version of the figure/table to the most recent version checked into the Git repository. If the user provides
a file path to the `.path_previous` argument, then the html page will show a comparison of the two local figures/tables.

Additionally, users can generate these comparisons on either a single file or directory of files. If a directory is 
given, the generated html page will contain all relevant figures/tables in the directory.
